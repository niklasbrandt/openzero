...
[![Self-Hosted Agent](https://img.shields.io/badge/Agent-Self--Hosted-0B3D91.svg)](https://github.com/niklasbrandt/openzero)
[![Local Data](https://img.shields.io/badge/Data-Local_Only-1766AD.svg)](#network-perimeter)
[![Self-Hosted](https://img.shields.io/badge/Deployment-Self--Hosted-217FBC.svg)](BUILD.md)
[![Vector Memory](https://img.shields.io/badge/Memory-Vector_Store-2D9CC7.svg)](#memory-connections)
[![Scheduled Agents](https://img.shields.io/badge/Agents-Scheduled-43B5CF.svg)](#crews)
...
**Sequential panel debates.** When you prompt Z with a complex query touching multiple overlapping domains, the system triggers a collaborative panel debate among the relevant experts. This works equally for business initiatives (e.g. aligning sprint deliverables and financial budgets with marketing campaigns) as it does for personal domains (e.g. balancing workload stress with physical recovery). The panel operates in a sequenced priority chain:
1. **Domain Experts** (e.g. market-intel, legal, and content in a business project; or health, fitness, and chef in a lifestyle context) run first, drafting their respective plans independently.
2. **Focus** (Clarity & Priority Synthesizer) runs next, reviewing the drafts, evaluating conflict points, checking calendar availability, and resolving contradictions to establish a singular focus plan. It maps all proposed tasks to long-term goals, triages them against daily resource or energy capacity, and uses strict leverage overrides (e.g. critical business deadlines down-scaling non-urgent tasks, or biological recovery vetoing high physical exertion) to output one cohesive strategy.
...
**Dynamic round scaling.** If Focus detects a critical contradiction or conflicting constraints that remain unresolved after the initial rebuttal round (e.g. legal insisting on a contract review that blocks a critical software release, or physical exertion plans ignoring a severe fatigue veto), it dynamically triggers a third **Reconciliation Round**. The conflicting crews are adaptively queried to resolve the contradiction safely under Focus's guidance, ensuring Z's final Executive Synthesis is perfectly aligned, cohesive, and actionable.
