...
# Emoji regex: match most common emoji ranges (Emoticons, Dingbats,
# Transport/Map, Misc Symbols, Supp Symbols, Flags, etc.).
	...
	try:
		...
		if tier_name == "deep" and settings.SMART_MODEL_INTERACTIVE:
			try:
				...
				first_chunk = await asyncio.wait_for(stream.__anext__(), timeout=settings.DEEP_MODEL_TIMEOUT_S)
				cleaned = _EMOJI_RE.sub("", first_chunk)
				if cleaned:
					yield cleaned
				async for chunk in stream:
					cleaned = _EMOJI_RE.sub("", chunk)
					if cleaned:
						yield cleaned
				return
			except (asyncio.TimeoutError, StopAsyncIteration):
