...
def _assemble_block(contents: dict[str, str]) -> str:
	...
	return (
		...
		"IMPORTANT: Values in square brackets such as [pig], [Omnivore], [80kg], [workdays] are\n"
		"ACTUAL user data — not template placeholders. Apply them directly and silently.\n"
		"When health, dietary, or personal constraints are present, embed them in your response\n"
		"without asking the user to confirm or specify them again.\n"
...
def _compute_hash(files: list[Path]) -> str:
	...
	for p in files:
		try:
			...
			logger.debug("Personal context file hash failed: %s", _oe)
	return h.hexdigest()
