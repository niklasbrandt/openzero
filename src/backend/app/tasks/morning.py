...
async def morning_briefing():
	...
	try:
		...
		full_prompt = (
			...
			"- PRONUNCIATION: You must mark ALL English words or technical terms so the TTS engine switches voices. To do this, put the EXACT literal string \"<en>\" before the English word, and the EXACT literal string \"</en>\" after it. Example: Wir brauchen ein <en>PC Case</en> für das <en>Dashboard</en>. Do not invent other tags (like <edible>). ONLY use <en> and </en>.\n"
		...
		try:
			...
			raw_content = await asyncio.wait_for(chat(full_prompt, tier="cloud", _feature="morning_briefing", max_tokens=2000, include_health=False), timeout=600.0)
		except asyncio.TimeoutError:
			logger.warning("morning_briefing — cloud tier timed out after 600s, retrying")
			raw_content = await chat(full_prompt, tier="cloud", _feature="morning_briefing", max_tokens=2000, include_health=False)
		...
		logger.info(f"TTS Text: {clean_text}")
