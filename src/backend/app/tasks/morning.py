...
async def morning_briefing():
	...
	try:
		...
		full_prompt = (
			...
			"Board (Today): [only if PROJECT TREE has real cards — act as a kanban expert: identify the 1-2 highest-value cards to pull or unblock today; flag anything stuck, overdue, or blocking something downstream; recommend the single next physical action for the top card]\n\n"
			...
			"WRONG (passive summary): '- Privacy dashboard: In Review. - openZero backend: In Progress.'\n"
			"RIGHT (kanban expert): '- Privacy dashboard has been in review 2 days — unblock it first, one test run closes it.\n- openZero backend: TURN fix is in progress, nothing else should jump the queue until that ships.'\n\n"
			...
			"- As a kanban expert, apply these principles to the board section:\n"
			"  * WIP discipline: if In Progress has multiple cards, flag that and recommend finishing one before pulling new work.\n"
			"  * Pull signal: identify the single highest-value card in Backlog or Todo that should be pulled to In Progress today.\n"
			"  * Blocker detection: if a card has been in the same list for 2+ days (infer from RECENT ACTIVITY), call it out and suggest the one action that would unblock it.\n"
			"  * Done sweep: if Done/Review has cards that should be archived or followed up, surface that.\n"
