				...
				<div class="llm-ram-breakdown">
					${tierNames.map((name) => {
						...
						return `<div class="llm-tier-card${externalActive ? ' peer-routed' : ''}" style="--tier-color:${color}" data-tier="${name}">
							...
							<div class="ltc-specs">
								${(!externalActive && fileSizeGb) ? `<div class="ltc-spec has-tip"><span>${this.tr('diag_llm_file', 'File')}</span><strong>${fileSizeGb.toFixed(1)} GB</strong><span class="glass-tooltip">${this.tr('diag_llm_file_tip', 'Physical size of the model file on disk.')}</span></div>` : ''}
