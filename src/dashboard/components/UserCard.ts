    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="card">
        ...
        : `<h2>${me.name || 'User'}</h2>`}
          ...
          ${!this.isEditing ? `<button class="edit-btn" id="edit-trigger">Edit</button>` : ''}
        </div>
        ...
        : `<ul>${(me.context || '').split('\n').filter((l: any) => l.trim()).map((l: any) => `<li>${l}</li>`).join('') || '<li>Provide goals to help Z prioritize.</li>'}</ul>`}
        </div>
