...
## Run 10 (code-split lazy components): Predicted Performance 85-95 / A11y 100

Changes from Run 9:
- Split 13 non-critical Web Components from synchronous to dynamic imports deferred via `requestIdleCallback` / `setTimeout(200ms)`.
- Critical path components kept synchronous: `ChatPrompt`, `LifeOverview`, `UserCard`.
- Added Vite `rollupOptions.manualChunks`: main JS bundle split into `index.js` (89 kB / 22 kB gzip) + `lazy-components.js` (183 kB / 38 kB gzip).
- Main JS bundle reduced from ~257 kB to 89 kB on the critical path.
- Expected TBT improvement: ~600 ms reduction (from 740 ms to target <150 ms on CI runner).

| Metric (target after Run 10)   | Run 9 (CI)  | Target     |
|:-------------------------------|:------------|:-----------|
| Total Blocking Time            | 740 ms      | < 150 ms   |
| First Contentful Paint         | 2.7 s       | ~2.0 s     |
| Largest Contentful Paint       | 3.8 s       | ~2.5 s     |
| Performance Score              | 67          | 85+        |

Bundle (post-split): main JS 89.34 kB (gzip 22.62 kB), lazy JS 182.92 kB (gzip 37.91 kB), CSS 26.59 kB (gzip 5.51 kB).

---

## Run 9 (CI Lighthouse -- Slow 4G / Moto G Power): Performance 67

Run 9 was generated by GitHub Actions CI against a local Vite preview build. Significant regression vs Run 8 local.

Root cause: 16 synchronous component imports created 10 long tasks totaling 5.8 s main-thread work on CI's slower CPU (Moto G Power simulation, Slow 4G). TBT spiked to 740 ms.

| Metric (CI, Simulated Mobile) | Value    | Score |
|:------------------------------|:---------|:------|
