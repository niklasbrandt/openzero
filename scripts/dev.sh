...
if ! docker compose up -d postgres qdrant llm-instant llm-standard llm-deep planka 2>&1; then
    echo -e "${RED}󱙝 Potential Docker corruption detected. Attempting repair...${RESET}"
    docker system prune -f > /dev/null 2>&1
    docker compose up -d postgres qdrant llm-instant llm-standard llm-deep planka
fi
...
cleanup() {
    ...
    docker compose stop postgres qdrant llm-instant llm-standard llm-deep planka > /dev/null 2>&1
