...
The crew registry lives in `agent/crews.yaml`. The example template includes crews across five domains:

**Research** — deep multi-vector research with credibility scoring and a contrarian audit pass built in.

**Health and lifestyle** — nutrition planning, fitness programming, weekly habit tracking, and sleep protocol optimization.

**Private and personal** — life guidance and coaching, nutrition planning, fitness programming, weekly health tracking, home maintenance, travel planning, child development logistics, and digital security.

Crews are not active by default. Uncomment and configure the ones relevant to your instance. Each crew specifies which briefing cadence it feeds (`feeds_briefing: "/day"`, `/week"`, `/month"`, etc.) and how many minutes before briefing time it should start. The scheduler handles the timing automatically.
